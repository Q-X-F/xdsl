...
INSTRUCTIONS = [
    "add", "sub", "mul", "div", "inc", "dec", "and", "or", "xor", "not", "neg",
    "push", "pop", "mov", "cmp", "lea",
...
def highlight_x86(code: str) -> str:
    ...
    for line in code.splitlines():
        ...
        if stripped.endswith(":"):
            label = stripped[:-1]
            highlighted_lines.append(indent + color(label + ":", "label") + comment_part)
            continue
        ...
        for token in tokens:
            ...
            if lower in INSTRUCTIONS:
                ...
                clean.startswith("0x") and all(c in "0123456789abcdefABCDEF" for c in clean[2:])
            ):
